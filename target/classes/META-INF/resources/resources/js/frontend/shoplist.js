$(function () {
    ...
    function addItems(pageSize,pageIndex) {
        //拼接出查询的URL，赋空值默认就去掉这个条件的限制，有值就代表按这个条件去查询店铺信息
        var url = listUrl + '?' + 'pageSize='+pageSize+'&pageIndex='+pageIndex+'&parentId='+parentId+
        ...
        $.getJSON(url,function (data) {
            if (data.success){
                ...
                //获取目前为止已显示的卡片总数，包括之前已经加载的
                var total = $('.list-div .card').length;
                //若总数达到跟安装此查询条件列出来的总数一致，则停止后台的加载
                if (total >= maxItems){
    ...
    $('#shoplist-search-div').on(
        ...
        function (e) {
            if (parentId){
                ...
                //若之前已选定好别的category，则移除其选定的效果，改成选定新的
    ...
    //需要查询的店铺名字发生变化后，重置页码，清空原先的店铺类别，按照新的名字去进行查询
    ...
    //区域信息变化后，重置页码，清空原先的店铺列表，按照新的区域去查询
